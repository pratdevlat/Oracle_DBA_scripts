...
FROM
    dba_users@remote_schema_link   a
...
FROM
    (select
       ...
       , decode(  object_type
                ...
                , object_name)
     from  dba_objects@remote_schema_link
     minus
     select
         object_type
       , decode(  object_type
...
(
  SELECT
    ...
    , os_granted
  FROM
      dba_role_privs@remote_schema_link
  MINUS
  SELECT
      granted_role
    , 'Remote' schema
...
(
  SELECT
    ...
    , admin_option
  FROM
      dba_sys_privs@remote_schema_link
  MINUS
  SELECT
      privilege
    , 'Remote' schema
...
(
  SELECT
    ...
    , grantable
  FROM
      dba_tab_privs@remote_schema_link
  WHERE
      (owner, table_name) IN (
...
SELECT
  ...
  , object_type
FROM
    dba_objects@remote_schema_link
ORDER BY object_type, object_name;
...
(
  ...
  WHERE  table_name IN (
             select table_name
             from   dba_tables@remote_schema_link
         )
  MINUS
  SELECT
      table_name
...
(
  SELECT
    ...
    , b.column_position
  FROM
      dba_indexes@remote_schema_link      a
    , dba_ind_columns@remote_schema_link  b
  WHERE
        a.index_name IN (
           select index_name
           from   dba_indexes
...
(
  ...
  WHERE
        a.index_name IN (
            select index_name
            from   dba_indexes@remote_schema_link
        )
    AND b.index_name = a.index_name
    AND b.table_name = a.table_name
  MINUS
...
DECLARE
    ...
    CURSOR c2 IS
        SELECT constraint_name, search_condition
        FROM   dba_constraints@remote_schema_link
        WHERE  search_condition IS NOT NULL;
...
(
  SELECT
    ...
    , b.object_text
  FROM
      dba_constraints@remote_schema_link  a
...
(
  SELECT
    ...
    , cache_size
  FROM
      dba_sequences@remote_schema_link
  MINUS
  SELECT
      sequence_name
    , 'Remote' schema
...
(
  SELECT
    ...
    , db_link
  FROM
      dba_synonyms@remote_schema_link
  MINUS
  SELECT
      synonym_name
    , 'Remote' schema
...
FROM
    ( (  SELECT   name, type, line, text
         FROM     dba_source@remote_schema_link
         WHERE    (name, type) IN (
...
DECLARE
    ...
    CURSOR c2 IS
        SELECT trigger_name, trigger_body
        FROM   dba_triggers@remote_schema_link;
...
( SELECT
  ...
  SELECT
    ...
    , b.hash_value
  FROM
      dba_triggers@remote_schema_link  a
    , schema_compare_temp               b
  WHERE
        b.object_name(+) = a.trigger_name
    AND b.database(+)    = 2
...
DECLARE
    ...
    CURSOR c1 IS
        SELECT view_name, getLongText('dba_VIEWS', 'TEXT', view_name)
        FROM   dba_views;

    CURSOR c2 IS
        SELECT view_name, getLongText2('dba_VIEWS', 'TEXT', view_name)
        FROM   dba_views@remote_schema_link;
...
(
  ...
  SELECT
    ...
    , b.hash_value
  FROM
      dba_views@remote_schema_link  a
    , schema_compare_temp            b
  WHERE
        b.object_name(+)  = a.view_name
    AND b.database(+)     = 2
...
(
  SELECT
    ...
    , host
  FROM
      dba_db_links@remote_schema_link
  MINUS
  SELECT
      db_link
    , 'Remote' schema
